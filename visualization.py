...
from cvrp import Solution, CVRPInstance
import numpy as np
...
def plot_comparison(solutions: List[Solution], titles: List[str] = None,
                   figsize: tuple = (16, 8)):
    """
    ...
    Args:
        ...
        titles: List of titles for each solution
        figsize: Figure size
    """
    n = len(solutions)
    if titles is None:
        titles = [f'Solution {i+1}' for i in range(n)]
    ...
    for idx, (solution, title) in enumerate(zip(solutions, titles)):
        ...
        # Plot depot
        ax.scatter(instance.depot.x, instance.depot.y, 
                  c='red', s=200, marker='s', edgecolors='black', linewidths=2, zorder=5)
        ...
        for route_idx, route in enumerate(solution.routes):
            ...
            for customer_id in route:
                customer = instance.customers[customer_id]
                ax.scatter(customer.x, customer.y, c=[color], s=80, 
                          edgecolors='black', linewidths=1, zorder=4)
        
        ax.set_title(f'{title}\nDistance: {solution.total_distance:.2f} | Routes: {len(solution.routes)}', 
...
def plot_solution_with_comparison(solution: Solution, best_distance: Optional[float] = None,
                                 title: str = "CVRP Solution", show_labels: bool = True,
                                 figsize: tuple = (12, 8)):
    """
    Plot solution with comparison to best-known distance
    
    Args:
        solution: The solution to visualize
        best_distance: Best-known distance for comparison
        title: Plot title
        show_labels: Whether to show customer IDs
        figsize: Figure size
    """
    ...
    # Calculate gap if best distance provided
    gap_text = ""
